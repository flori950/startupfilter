...
def categorize_company(row):
    ...
    # Check for all matching RE strategies
    for key, strategy in Keywords.re_strategies_1.items():
        if strategy.lower() in description:
            matched_categories.append(strategy)

    # If one or more matches are found, return them as a comma-separated string
    if matched_categories:
        return pd.Series([row['Name'], row['Short_Description'], ', '.join(matched_categories), len(matched_categories)])
    else:
        # Return 'Uncategorized' if no matches are found
        return pd.Series([row['Name'], row['Short_Description'], 'Uncategorized', 0])
...
def run_job():
    ...
    df[['Company_Name', 'Short_Description', 'Categorization', 'Category_Count']] = df.apply(categorize_company, axis=1)
